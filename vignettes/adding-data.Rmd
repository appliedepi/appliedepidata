...
3.  **Build the package with the added data**
    ...
    A. Navigate to the Menu/Title bar, select 'Build', and then click 'Install package' (Ctrl + Shift + B) to build the package with the newly added
...
6.  **Add documentation for each dataset added**
    ...
    D. Run `devtools::document()` in the console to create the actual R documentation.
...
6.  **Add the datasets to `_pkgdown.yml`**
    ...
    C. Make sure to have correct indentation, use of dashes and lines between entries. See
...
-   **type_version**: Used to identify the *original* dataset and its
    ...
    associated with a parent. If there are multiple linelists in one 
    group (which aren't originated from eachother), this would 
    increment with the `type` column.
...
-   **usage**: Intended usage for the data. For example courses should be 
    ...
    case study id (e.g. `m̀ultidisease_surveillance`, `r_practical`). 
    A resource should just be the name (e.g. `epirhandbook`, `aem`). 
    Can be multiple tags separated by commas.
