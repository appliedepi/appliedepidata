...
dict_vars <- rio::import(temp_file, which = "lab_variables")
dict_vals <- rio::import(temp_file, which = "lab_values")
...
for (i in langs) {
  ...
  appropriate_filename <- dict_dataset_names[
    dict_dataset_names$dataset_name == "multidisease_tests", i]
  ...
  rio::export(generic_data, paste0("inst/extdata/", appropriate_filename, ".csv"))
