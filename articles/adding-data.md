...
3.  **Build the package with the added data**
    ...
    A. Navigate to the Menu/Title bar, select ‘Build’, and then click
    ‘Install package’ (Ctrl + Shift + B) to build the package with the
    newly added data. This will mean the data is recognized for the next
    step.
...
- **type_version**: Used to identify the *original* dataset and its
  associated child data. Increment when there is a new dataset, not
  associated with a parent. If there are multiple linelists in one group
  (which aren’t originated from eachother), this would increment with
  the `type` column.
...
- **usage**: Intended usage for the data. For example courses should be
  the coursecode (e.g. `intror` or `qmd`). Case studies should be the
  case study id (e.g. `m̀ultidisease_surveillance`, `r_practical`). A
  resource should just be the name (e.g. `epirhandbook`, `aem`). Can be
